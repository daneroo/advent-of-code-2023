...
export async function day18b(dataPath?: string) {
  ...
  data.forEach((line: string) => {
    // console.log({ line });
    const [_dir, lenStr, colorStr] = line.split(' ');
    const _len = parseInt(lenStr);
    ...
    const len = parseInt(color.slice(1, 6), 16);
    const dir = ['R', 'D', 'L', 'U'][color.slice(-1)];
    // const colorDir = {}[color.slice(-1)];
